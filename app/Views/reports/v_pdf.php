...
<body>
  ...
  <div class="header">
    <table style="width: 100%;">
      <tr>
        <td>
          <h2>OPTIKERS Laporan Penjualan</h2>
          <p>Kategori: <strong><?= $categoryText ?></strong> | Periode: <strong><?= $startDate ?> s/d <?= $endDate ?></strong></p>
        </td>
  ...
  <table style="width: 100%; margin-bottom: 20px;">
    <tr>
      <td class="summary-card">
        <div class="title"><?= $category === 'refund' ? 'Total Refund' : ($category === 'cancellation' ? 'Total Dibatalkan' : 'Total Pendapatan') ?></div>
        <div class="value" style="color: #2FB8AA;">Rp <?= number_format($summary['total_revenue'], 0, ',', '.') ?></div>
      </td>
      <td style="width: 2%;"></td>
      <td class="summary-card">
        <div class="title"><?= $category === 'refund' ? 'Total Retur' : ($category === 'cancellation' ? 'Total Pembatalan' : 'Total Transaksi') ?></div>
        <div class="value" style="color: #2dce89;"><?= number_format($summary['total_transactions'], 0, ',', '.') ?></div>
      </td>
      <td style="width: 2%;"></td>
      <td class="summary-card">
        <div class="title"><?= $category === 'refund' ? 'Item Direfund' : ($category === 'cancellation' ? 'Item Dibatalkan' : 'Item Terjual') ?></div>
  ...
  <h3 style="margin: 0 0 10px 0; color: #333; text-transform: uppercase; font-size: 12px;">Rincian Transaksi Penjualan</h3>
  <table class="data-table">
    <thead>
      <tr>
        ...
        <th>Customer</th>
        <th>Email</th>
        <th class="text-center" style="width: 70px;">Total Item</th>
  ...
  <!-- FOOTER -->
  <div class="footer">
    Laporan Penjualan Otikers - Dokumen ini dibuat otomatis oleh sistem. Halaman 1 dari 1
  </div>
