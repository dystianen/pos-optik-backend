  ...
  <div class="card-body">
    ...
    <form action="<?= site_url('inventory/save') ?>" method="post" novalidate>
      <input type="hidden" name="id"
        value="<?= isset($transaction) ? htmlspecialchars($transaction['inventory_transaction_id']) : '' ?>">
      ...
      <div class="row">
        ...
        <div class="col-12 col-md-6 mb-3">
          ...
          <select <?= isset($transaction) ? "disabled" : "" ?> class="form-control" <?= !isset($transaction) ? 'name="product_id"' : '' ?> id="product_id" <?= !isset($transaction) ? 'required' : '' ?>>
            <option value="">-- Select a Product --</option>
            <?php foreach ($products as $product): ?>
              <option value="<?= htmlspecialchars($product['product_id']) ?>" <?= isset($transaction) && $transaction['product_id'] == $product['product_id'] ? 'selected' : '' ?>>
        ...
        <div class="col-12 col-md-6 mb-3" id="variant_wrapper" style="display:none;">
          ...
          <select <?= isset($transaction) ? "disabled" : "" ?> class="form-control" <?= !isset($transaction) ? 'name="variant_id"' : '' ?> id="variant_id">
        ...
        <div class="col-12 col-md-6 mb-3">
          ...
          <select class="form-control" name="transaction_type" id="transaction_type" required>
            ...
            <option value="in" <?= isset($transaction) && $transaction['transaction_type'] === 'in' ? 'selected' : '' ?>>IN
              (Stock In)</option>
            <option value="out" <?= isset($transaction) && $transaction['transaction_type'] === 'out' ? 'selected' : '' ?>>
              OUT (Stock Out)</option>
          </select>
          <div class="invalid-feedback">Please select a transaction type.</div>
          <small class="form-text text-muted d-block mt-1">Indicate whether this is a stock increase (IN) or decrease
            (OUT)</small>
        </div>
        ...
        <div class="col-12 col-md-6 mb-3">
          ...
          <small class="form-text text-muted d-block mt-1">Optional: Link this transaction to an order, adjustment, or
            process</small>
        </div>
        ...
        <div class="col-12 col-md-6 mb-3">
          ...
          <small class="form-text text-muted d-block mt-1">Enter the quantity to be added or removed (must be greater
            than 0)</small>
        </div>
        ...
        <div class="col-12 mb-3">
          ...
          <textarea class="form-control" name="description" id="description" rows="4"
            placeholder="Enter notes or details about this transaction..."><?= old('description', isset($transaction) ? htmlspecialchars($transaction['description']) : '') ?></textarea>
...
<script>
  $(document).ready(function () {
    ...
    let initialProduct = $('#product_id').val();
